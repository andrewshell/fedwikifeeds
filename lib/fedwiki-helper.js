...
async function fetchSitemap(domain, cachePref) {
  ...
  const now = Date.now();
  const filterDate = new Date(now - (7 * 24 * 60 * 60 * 1000)); // One Week
  const futureDate = new Date(now); // Ignore pages dated in the future (bad data)
  const defaultPageSlugs = ['federated-wiki', 'how-to-wiki', 'smallest-federated-wiki', 'welcome-visitors'];
  ...
  const data = await Promise.all(c.data.filter((page) => {
    ...
    return pageDate > filterDate && pageDate <= futureDate && !defaultPageSlugs.includes(page.slug);
  }).map(async (page) => {
