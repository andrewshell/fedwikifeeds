...
async function fetchAllFeeds() {
  ...
  allFeedsCache = await cache.decorate('-internal', 'allfeeds.json', cache.status.preferCache, () => {
    return {};
  });
...
async function saveAllFeeds(data) {
  return await cache.decorate('-internal', 'allfeeds.json', cache.status.onlyFresh, _filterBlackList.bind(null, data));
}
...
async function fetchAllRosters() {
  ...
  allRostersCache = await cache.decorate('-internal', 'allrosters.json', cache.status.preferCache, () => {
    return {};
  });
...
async function saveAllRosters(data) {
  return await cache.decorate('-internal', 'allrosters.json', cache.status.onlyFresh, () => { return data; });
}
...
async function fetchSiteFeed(domain, cachePref) {
  ...
  return cache.decorate(domain, 'feed.json', cachePref, async (existing) => {
