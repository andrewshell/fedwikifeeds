...
function cache(name, status, callback) {
    ...
    try {
        ...
        if (status >= cache.status.preferCache && cache.store.isset(name)) {
            response = cache.store.get(name);
        } else if (status === cache.status.onlyCache) {
            throw new Error('Missing cache');
        } else {
            response = new cacheStoreCommon.Hit(name, callback());
            cache.store.set(response);
        }
    } catch (err) {
        if (status >= cache.status.cacheOnFail && cache.store.isset(name)) {
            response = cache.store.get(name);
            response.error = err;
        } else {
