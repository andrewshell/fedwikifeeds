...
function typeSanitizer(type) {
  switch (type) {
    case 'markdown':
      return (text) => sanitizeHtml(expand(text));
    case 'html':
      return sanitizeHtml;
    default:
...
async function fetchRiver(name, domains) {
  ...
  // Push the final accumulated feed block; the loop only flushes when the domain
  // changes, so without this the last group (or the only group) would be lost.
  if (null != feed) {
    river.updatedFeeds.updatedFeed.push(feed);
  }
