...
function resolveLinks(string, baseurl) {
  var external, internal, stash, stashed, unstash, sanitize;
  stashed = [];
  stash = function(text) {
    var here;
    here = stashed.length;
  ...
  return string.replace(/〖(\d+)〗/g, unstash);
};
...
async function fetchRiver(name, domains) {
  ...
  for (let item of items) {
    ...
    feed.item.push({
      "title": item.title,
      "link": item.link,
      "body": resolveLinks(item.text, item.link),
