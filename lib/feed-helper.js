...
async function fetchAllFeeds() {
  ...
  return pageCache.write(config.docroot, `allfeeds.opml`, opml.stringify(opmlstruct));
}

async function fetchActiveFeeds() {
  const allfeeds = Object.values((await fedwikiHelper.fetchAllFeeds()).data)
      .filter(filter => { return true === filter.active; })

  const opmlstruct = {
    opml: {
      head: {
        title: `All Federated Wiki Feeds`,
        length: allfeeds.length
      },
      body: {
        subs: allfeeds
      }
  ...
  return pageCache.write(config.docroot, `activefeeds.opml`, opml.stringify(opmlstruct));
}

async function fetchPeersOpml(domain, preferCache) {
  console.log(`fetchPeersOpml: ${domain}`);

  let cache = await pageCache.read(config.docroot, `${domain}/peers.opml`);
  ...
  if (false !== peers && (false === peers.cached || false === cache)) {
    ...
    cache = pageCache.write(config.docroot, `${domain}/peers.opml`, opml.stringify(opmlstruct));
...
async function fetchSiteRss(domain, preferCache) {
  ...
  let cache = await pageCache.read(config.docroot, `${domain}//rss.xml`);
  ...
  if (false !== sitemap && (false === sitemap.cached || false === cache)) {
    ...
    cache = await pageCache.write(config.docroot, `${domain}/rss.xml`, rss.buildRssFeed(headElements, historyArray));
