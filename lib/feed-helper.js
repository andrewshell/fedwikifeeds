...
async function fetchRiver(name, domains) {
  ...
  for (let domain of domains) {
    if (null != allfeeds.data[domain] && allfeeds.data[domain].active) {
      feed = await fedwikiHelper.fetchSiteFeed(domain, cache.status.onlyCache);
...
async function fetchPeersOpml(domain, cachePref) {
  ...
  return cache.decorate(domain, `peers.opml`, cachePref, async (existing) => {
...
async function fetchRosterOpml(domain, page, cachePref) {
  ...
  return cache.decorate(domain, `${page}/roster.opml`, cachePref, async (existing) => {
